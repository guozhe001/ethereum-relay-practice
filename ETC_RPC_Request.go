...
func NewETCRPCRequest(chainInfo constant.ChainInfo) *ETHRPCRequest {
  return &ETHRPCRequest{nonceManager: NewNonceManager(), client: NewETHRPCClient(chainInfo.NodeUrl), chainId: chainInfo.ChainId}
}
...
func (e *ETHRPCRequest) SendTransaction(address string, transaction *types.Transaction) (string, error) {
  // 签名交易数据
  signTx, err := wallet.SignETHTransaction(address, transaction)
  if err != nil {
  ...
  fmt.Printf("rlp序列化后的交易=%#v\n", txRlpData)

  hexData := hexutil.Encode(txRlpData)
  fmt.Printf("转换成十六进制的交易=%#v\n", hexData)
  // 下面调用以太坊的 rpc 接口
  txHash := ""
  methodName := "eth_sendRawTransaction"

  err = e.client.client.Call(&txHash, methodName, hexData)
  if err != nil {
...
func (e *ETHRPCRequest) SendETHTransaction(fromStr, toStr, valueStr string, gasLimit, gasPrice uint64) (string, error) {
  ...
  realV := util.GetRealDecimalValue(valueStr, constant.DefaultDecimal)
  if realV == "" {
    return "", errors.New("invalid value")
  }
  amount, _ := new(big.Int).SetString(realV, 10)
  nonce, err := e.getNonce(fromStr)
  if err != nil {
    return "", err
  }
  // 构建交易结构体
  transaction := newTx(nonce.Uint64(), &to, amount, gasPrice_, gasLimit, []byte(""), e.chainId)

  return e.SendTransaction(fromStr, transaction)
}

// getNonce 先从缓存中获取nonce，如果获取不到则从链上获取并更新缓存
func (e *ETHRPCRequest) getNonce(address string) (*big.Int, error) {
  // 获取 nonce
  nonce := e.nonceManager.GetNonce(address)
  if nonce == nil {
    // nonce 不存在，开始访问节点获取
    n, err := e.GetNonce(address)
    if err != nil {
      return big.NewInt(0), fmt.Errorf("获取 nonce 失败 %s", err.Error())
    }
    nonce = new(big.Int).SetUint64(n)
    e.nonceManager.SetNonce(address, nonce)
  }
  return nonce, nil
}

// newTx 新建交易
func newTx(nonce uint64, to *common.Address, value, gasPrice *big.Int, gas uint64, data []byte, chainId *big.Int) *types.Transaction {
